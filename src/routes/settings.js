...
router.get('/:portalId', requirePortal, async (req, res) => {
  try {
    ...
    res.json({ triggerStages, reservationProbability, stages });
  } catch (err) {
...
// POST /api/settings/:portalId — save trigger stages and reservation probability
router.post('/:portalId', requirePortal, async (req, res) => {
  const { triggerStages, reservationProbability } = req.body;
  ...
  try {
    await db.execute({
      sql: `INSERT INTO portal_settings (portal_id, trigger_stages, reservation_probability, updated_at)
            VALUES (?, ?, ?, ?)
            ON CONFLICT(portal_id) DO UPDATE SET
              trigger_stages = excluded.trigger_stages,
              reservation_probability = excluded.reservation_probability,
              updated_at = excluded.updated_at`,
      args: [req.portalId, JSON.stringify(triggerStages), prob, now],
    });
    res.json({ ok: true, triggerStages, reservationProbability: prob });
  } catch (err) {
